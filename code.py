...
Convolution2D, MaxPooling2D, GlobalMaxPool2D
...
# return data-0.5
def preprocess_audio_mel_T(audio, sample_rate=16000, window_size=20,
#log_specgram
...
# In[4]:
file_to_label = {"../input/audio_train/"+k:v for k,v in
...
def get_model_mel():
 ...
 img_1 = Convolution2D(16, kernel_size=(3, 7),
activation=activations.relu)(norm_inp)
 img_1 = Convolution2D(16, kernel_size=(3, 7),
activation=activations.relu)(img_1)
 img_1 = MaxPooling2D(pool_size=(3, 7))(img_1)
 img_1 = Dropout(rate=0.1)(img_1)
 img_1 = Convolution2D(32, kernel_size=3,
activation=activations.relu)(img_1)
 img_1 = Convolution2D(32, kernel_size=3,
activation=activations.relu)(img_1)
 img_1 = MaxPooling2D(pool_size=(3, 3))(img_1)
 img_1 = Dropout(rate=0.1)(img_1)
 img_1 = Convolution2D(128, kernel_size=3,
activation=activations.relu)(img_1)
 img_1 = GlobalMaxPool2D()(img_1)
 img_1 = Dropout(rate=0.1)(img_1)
 dense_1 = BatchNormalization()(Dense(128,
activation=activations.relu)(img_1))
 dense_1 = BatchNormalization()(Dense(128,
activation=activations.relu)(dense_1))
...
# In[15]:
tr_files, val_files = train_test_split(sorted(train_files), test_size=0.1,
random_state=42)
# In[16]:
model = get_model_mel()
...
steps_per_epoch=len(tr_files)//batch_size, epochs=20,
# validation_data=train_generator(val_files),
validation_steps=len(val_files)//batch_size,
# use_multiprocessing=True, workers=8,
max_queue_size=20,
#
callbacks=[ModelCheckpoint("baseline_cnn_mel_bigger.h5", monitor="val_acc",
save_best_only=True),
...
for i in tqdm(range(bag)):
 list_preds = []
 for batch_files in tqdm(chunker(test_files, size=batch_size),
total=len(test_files)//batch_size ):
 batch_data = [load_audio_file(fpath) for fpath in batch_files]
 batch_data = np.array(batch_data)[:, :, :,np.newaxis]
...
#https://www.kaggle.com/inversion/freesound-starter-kernel
